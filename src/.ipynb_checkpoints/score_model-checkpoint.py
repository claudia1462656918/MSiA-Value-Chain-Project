...
def prediction_helper(obs, cutoff):
    '''
    # Given an observation, predict whether the customers buy the financial product or not based on the cutoff point
    
    Args: 
        obs(:py:class:`pandas.DataFrame`): an observation with the column label pred_prob
        cutoff(int): probability cutoff point, above which the customer is classifed as buying the product 
    
    Returns:
        A integer number [0,1]
    '''
    logger.info("Our threshold value for probability cutoff is {}".format(cutoff))
    
    if obs['pred_prob'] > cutoff:
        return 1
    else:
        return 0
...
def score_model(df, path_to_tmo, cutoff, save_scores=None, **kwargs):
    ...
    y_predicted['pred'] = y_predicted.apply (lambda obs: prediction_helper(obs, cutoff), axis=1)
